      ...
      switch (type) {
        case 'repos':
          ...
          var repoOwnedByOrg = (rd.full_name.toLowerCase().indexOf(config.orgName.toLowerCase() + '/') === 0),
              notBlacklisted = (config.blacklist.repos.indexOf(rd.name) === -1);
          if (repoOwnedByOrg && notBlacklisted) {
            resourceData[rd.id] = rd.name;
          }
          break;
        case 'members':
          notBlacklisted = (config.blacklist.members.indexOf(rd.login) === -1);
          if (notBlacklisted) {
...
var addTeamUsers = function(userKeys, allUsers, callback) {
  ...
  if (user) {
    ghteam.addUser(user, function() {
      log.info('added', user);
      if (userKey.length > 0) {
        addTeamRepos(userKeys, allUsers, callback);
      } else {
        callback();
      }
    });
